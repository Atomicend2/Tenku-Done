      ...
      <div className="rounded-xl relative overflow-hidden border border-primary/20 shadow-[0_0_60px_rgba(14,165,233,0.08)]">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div className="-mt-20 md:-mt-24 shrink-0 ring-4 ring-black rounded-full">
            ...
            />
            <div className="hidden w-24 h-24 md:w-32 md:h-32 rounded-full bg-black border-2 border-primary shadow-[0_0_20px_rgba(14,165,233,0.4)] items-center justify-center text-4xl md:text-5xl font-serif text-primary">
  ...
  const handleUpload = async (type: "pp" | "bg") => {
    ...
    try {
      ...
      if (json.success) {
        ...
        queryClient.invalidateQueries({ queryKey: ["user-stats"] });
        // Force the image to reload by appending a timestamp query param
        const ts = Date.now();
        if (type === "pp") {
          setPpPreview(`/api/v1/user/avatar?t=${ts}`);
        } else {
          setBgPreview(`/api/v1/user/background?t=${ts}`);
        }
