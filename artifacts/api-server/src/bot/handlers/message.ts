...
export async function handleMessage(
    ...
    try {
      ...
      if (lidRecord) {
        ...
        if (!phoneRecord) {
          ...
          // Both records exist — link the real record's LID column for future fast lookups
          db.prepare("UPDATE users SET phone = ?, lid = COALESCE(lid, ?) WHERE id = ? AND (phone IS NULL OR phone = '' OR phone = id)").run(realPhone, lidNum, lidNum);
          // Also store LID on the phone-keyed record so web OTP can find it
  ...
  if (!isGroup && !isOwner && !getStaff(sender)) {
    const allowedDmCmds = new Set(["register","reg","profile","p","balance","bal","daily","help","info","ping","alive","test","community","website"]);
  ...
  if (isGroup && !msg.key.fromMe) {
    // Clear AFK for any real content: text, media, or quoted replies (> Hello).
    // Only stickers and reactions are exempt because they are passive interactions.
    ...
    if (!isSticker && !isReaction && hasContent) {
